package de.nulldesign.nd2dx.support 
{
	 ...
	 * @author Thomas John
	 */
	public class _test_Test2DBatchRenderSupport extends RenderSupportBase
	{
		...
		public function _test_Test2DBatchRenderSupport() 
		{
			maxCapacity = 1000;
		...
		override public function drawMesh(meshRenderer:Mesh2DRendererComponent):void 
		{
			...
			*/
			...
			c = 0.0;
			d = material.height;
			tx = material.frameOffsetX;
			ty = material.frameOffsetY;
			...
			c = c1 * a2 + d1 * c2;
			d = c1 * b2 + d1 * d2;
			tx = tx1 * a2 + ty1 * c2 + node.tx;
			ty = tx1 * b2 + ty1 * d2 + node.ty;
			
			//var tempX:Number = (a1*point.x) + (b1*point.y) + (c1);
            //var tempY:Number = (a2*point.x) + (b2*point.y) + (c2);
			
			//var tempX:Number = (a*point.x) + (c*point.y) + (tx);
            //var tempY:Number = (b*point.x) + (d*point.y) + (ty);
			
			// v1
			mVertexBuffer[int(vIdx)] = a * v1x + c * v1y + tx;
			mVertexBuffer[int(vIdx + 1)] = b * v1x + d * v1y + ty;
			
			// v2
			mVertexBuffer[int(vIdx + 8)] = a * v2x + c * v2y + tx;
			mVertexBuffer[int(vIdx + 9)] = b * v2x + d * v2y + ty;
			
			// v3
			mVertexBuffer[int(vIdx + 16)] = a * v3x + c * v3y + tx;
			mVertexBuffer[int(vIdx + 17)] = b * v3x + d * v3y + ty;
			
			// v4
			mVertexBuffer[int(vIdx + 24)] = a * v4x + c * v4y + tx;
			mVertexBuffer[int(vIdx + 25)] = b * v4x + d * v4y + ty;
